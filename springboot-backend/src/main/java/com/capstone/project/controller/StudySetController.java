...
public class StudySetController {
    ...
    @GetMapping("/filterstudysetsclass")
        public ResponseEntity<?> getFilterListByClass( @RequestParam(value = "author_id", required = false, defaultValue = "0") int authorId,
                                                        ...
                                                        @RequestParam(value = "category_id", required = false, defaultValue = "0") int categoryId,
                                                        ...
                                                        @RequestParam(value = "size", required = false, defaultValue = "5") int size) {
        try {
            return ResponseEntity.ok(studySetService.getFilterListByClass(authorId, classId, categoryId, search,assigned, sortBy, direction, page, size));
        }
        catch (Exception e) {
            if(e.getMessage().equals("Please provide valid page and size")) {
                return ResponseEntity.badRequest().body(e.getMessage());
