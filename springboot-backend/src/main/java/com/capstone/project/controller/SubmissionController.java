...
@Autowired
    ...
    public ResponseEntity<?> getFilterList(@RequestParam(value = "search", required = false) String search,
                                           ...
                                           @RequestParam(value = "done", required = false) Boolean isDone,
        ...
        try{
            return ResponseEntity.ok(submissionService.getFilterSubmission(search,authorId.orElse(0),assignmentId.orElse(0),isDone,mark.orElse(0.0),from,to,direction,page,size));
        }catch (ResourceNotFroundException e){
            return ResponseEntity.badRequest().body(e.getMessage());
        }
    }
