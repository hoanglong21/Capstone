...
//    }
    ...
    @Order(7)
    @ParameterizedTest(name = "index => search={0},authorId{1}, assignmentId{2},mark{3}, fromCreated{4},toCreated{5}, direction{6}, page{7}, size{8}")
    @CsvSource({
            "Homework1,1,1,8.2,2023-8-9,2023-8-15,DESC,1,5",
            "Homwork2,2,1,5.5,2023-8-9,2023-8-15,DESC,1,5"
    })
    public void testGetFilterSubmission(String search, int authorId, int assignmentId, double mark, String fromCreated, String toCreated, String direction, int page, int size) throws ResourceNotFroundException {
        ...
        List<Submission> list = (List<Submission>) submissionServiceImpl.getFilterSubmission(search, authorId, assignmentId, mark, fromCreated, toCreated,
                direction, page, size).get("list");
