  ...
  useEffect(() => {
    const fetchData = async (tempType) => {
      ...
      try {
        ...
        setStudySet(temp);
        // type
        setType(Number(searchParams.get("type")) || temp.studySetType.id);
        // cards
        var tempCards = (await CardService.getAllByStudySetId(temp.id)).data;
        for (var card of tempCards) {
          ...
          card.studySet.user.created_date = toBEDate(tempUserCreatedDate);
        }
        setCards(tempCards);
      } catch (error) {
  ...
  const handleAddCard = async () => {
    ...
    try {
      if (cards.length < 500) {
        ...
        setError("The number of cards of a study set must not exceed 500");
      }
    } catch (error) {
      if (error.response && error.response.data) {
        console.log(error.response.data);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!studySet.title) {
        ...
        const emptyCards = (await StudySetService.checkStudySet(studySet.id))
          .data;
      ...
      <form className="mt-2 needs-validation" noValidate>
          ...
          {studySet._draft ? (
            <div className="container d-flex justify-content-between">
              <div className="d-flex">
                <h3 className="fw-bold create-set">{t('newSet')}</h3>
                {loading && <div className="createTest_status">Loading</div>}
        ...
        <div className="container mt-4">
          ...
          <div className="row">
            ...
            {/* is_public */}
            <div className="form-group mb-3 col-6">
              <label className={styles.formLabel}>{t('access')}</label>
              <select
            ...
            if (type === 1) {
              return (
                <VocabCard
                  ...
                  index={index}
                  card={card}
                  handleDelete={(event) => handleDelete(event, card)}
        ...
        >
          <Toast.Body className="d-flex flex-column p-3">
            <div className="d-flex justify-content-between">
              <span className="me-auto">{t('auto-saved')}.</span>
              <button
                type="button"
            ...
            <div>
              <button className="btn text-white" onClick={handleDiscard}>
              {t('discard')}
              </button>
            </div>
          </Toast.Body>
