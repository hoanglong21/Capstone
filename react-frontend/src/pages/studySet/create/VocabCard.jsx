...
export const VocabCard = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        var contents = (await ContentService.getAllByCardId(card.id)).data;
        if (contents.length === 0) {
          ...
          for (var content of contents) {
            const tempSetCreatedDate = content.card.studySet.created_date;
            content.card.studySet.created_date =
              tempSetCreatedDate.replace(/\s/g, "T") + ".000" + "+07:00";
            const tempUserCreatedDate = content.card.studySet.user.created_date;
  ...
  return (
    ...
    >
      ...
      >
        <span className={styles.card_header_label}>{props.index + 1}</span>
