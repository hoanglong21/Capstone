  ...
  useEffect(() => {
    if (ques?.question_type) {
      ...
      if (ques?.question_type === 3) {
        setCorrectAnswer(ques.question.card.id === ques.answers[0].card.id);
      }
    }
  }, [ques]);
    ...
    <div className="card learnQuestionCard">
      ...
      {ques.question_type === 1 && (
        <div className="card-body d-flex flex-column">
                    ...
                    <div className="row">
                      <div className="col-11">
                        <div
                          ...
                          dangerouslySetInnerHTML={{
                            __html: ques?.question?.progress?.note || "...",
                          }}
                        ></div>
                      </div>
                    </div>
          ...
          {isCurrentCorrect === false && (
            <div>
              <div className="quizQues_label my-4">{t("correctans")}</div>
      ...
      {ques.question_type === 2 && (
        <div className="card-body">
          {/* question */}
          {ques.question.content.map((itemContent, index) => {
            if (multiplePromptWith?.includes(itemContent.field.id)) {
              return (
                <div key={index} className="mb-2">
                  <div className="quizQues_label quizQues_label--sm mb-1">
                    {itemContent.field.name}
              ...
              <div key={ansIndex} className="col-12 mb-3">
                <div
                  className={`quizQues_answer ${
                    currentAnswer === ans.card.id && isCurrentCorrect === false
                      ? "incorrect"
                ...
                >
                  {ans.content.map((itemContent, index) => {
                    if (multipleAnswerWith?.includes(itemContent.field.id)) {
                      return (
                        <div key={index} className="mb-2">
        ...
        <div className="card-body d-flex flex-column">
            ...
            <div className="row mb-4">
              <div className="col-6">
                <div className="quizQues_question--left h-100">
                  {ques.question.content.map((itemContent, index) => {
                    if (trueFalsePromptWith?.includes(itemContent.field.id)) {
                      return (
                        <div key={index} className="mb-2">
                          <div className="quizQues_label quizQues_label--sm mb-1">
                            {itemContent.field.name}
              ...
              <div className="col-6">
                <div className="quizQues_question--right h-100">
                  {ques.answers[0].content.map((itemContent, index) => {
                    if (trueFalseAnswerWith?.includes(itemContent.field.id)) {
                      return (
                        <div key={index} className="mb-2">
                          <div className="quizQues_label quizQues_label--sm mb-1">
                            {itemContent.field.name}
          ...
          <div className="row">
            <div className="col-6">
              <div
                className={`quizQues_answer ${
                  currentAnswer === true && isCurrentCorrect === false
                    ? "incorrect"
                    : currentAnswer === true && isCurrentCorrect === true
                    ? "correct"
                    : ""
                }`}
                disabled={isCurrentCorrect !== null}
            ...
            <div className="col-6">
              <div
                className={`quizQues_answer ${
                  currentAnswer === false && isCurrentCorrect === false
                    ? "incorrect"
                    : currentAnswer === false && isCurrentCorrect === true
                    ? "correct"
                    : ""
                }`}
                disabled={isCurrentCorrect !== null}
