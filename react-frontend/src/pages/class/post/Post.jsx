...
const Post = ({ post, stateChanger, posts, index, userInfo }) => {
  ...
  const handleUpdatePost = async () => {
    ...
    try {
      // update post
      const tempPost = (await PostService.updatePost(updatePost.id, updatePost))
        .data;
      ...
      <div className="card mb-4">
        <div className="card-header bg-white d-flex align-items-center justify-content-between">
          <div className="d-flex align-items-center">
            ...
            <div className="ms-3">
              <div className="d-flex align-items-center">
                <Link to={`/${post.user.username}/sets`} className="postAuthor">
                  {post.user.username}
                </Link>
                {post?.user?.status === "banned" && (
                  <OverlayTrigger
                    placement="bottom"
                    overlay={
                      <Tooltip id="tooltip">This account is banned.</Tooltip>
                    }
                  >
                    <img
                ...
                {post?.user?.status === "active" && (
                  <OverlayTrigger
                    placement="bottom"
                    overlay={
                      <Tooltip id="tooltip">This account is verified.</Tooltip>
                    }
                  >
                    <img className="ms-1 author-avatarTag" src={verified} />
                  </OverlayTrigger>
                )}
                {post?.user?.status === "deleted" && (
                  <OverlayTrigger
                    placement="bottom"
                    overlay={
                      <Tooltip id="tooltip">This account is deleted.</Tooltip>
                    }
                  >
                    <img className="ms-1 author-avatarTag" src={deleted} />
                  </OverlayTrigger>
                )}
              </div>
              <div className="postCreatedDate">{post.created_date}</div>
            </div>
          </div>
            ...
            <div className="dropdown">
              ...
              <ul className="dropdown-menu">
                <li>
                  ...
                  >
                    <span className="align-middle fw-medium">Edit</span>
                  </button>
                </li>
                <li>
                  <button
                  ...
                  >
                    <span className="align-middle fw-medium">Delete</span>
                  </button>
                </li>
              </ul>
            </div>
                    ...
                    <div className="card mb-2" key={index}>
                      <div className="card-body d-flex justify-content-between">
                        <a
                          ...
                          target="_blank"
                        >
                          <div className="fileUploadName">{file.file_name}</div>
                          <div className="fileUploadType">{file.file_type}</div>
                        </a>
                        <button
                ...
                <div className="d-flex align-items-center justify-content-between mt-4">
                  ...
                  <div className="d-flex align-items-center">
                    ...
                    <button
                      ...
                      disabled={!updatePost.content || loadingUpdatePost}
                    >
          ...
          {comments.length > 0 && (
            <div>
              <div className="d-flex align-items-center comment_label mb-3">
                ...
                <span>{comments.length} {t('class')} {t('comment')}</span>
              </div>
                ...
                >
                  {loadingComment ? (
                    <div
                      ...
                      role="status"
                    >
                      <span className="visually-hidden">LoadingUpload...</span>
                    </div>
                  ) : (
